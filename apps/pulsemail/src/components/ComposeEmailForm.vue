  ...
  if (to.value.trim()) {
    try {
      ...
      const emailData = {
        ...
        isReply: !!props.replyTo && !props.forward,
        isForward: !!props.forward,
        originalEmail: props.replyTo
      };
      
      // Add to sent emails in the store
      const sentEmail = emailStore.addToSent(emailData);
      
      // Emit the send event with the email data
      emit('send', sentEmail);
    } catch (error) {
      console.error('Error sending email:', error);
      // In a real app, this would handle errors and pass them to the parent
      emit('error', 'Failed to send email');
    }
  }
};
